...
const sucursalesPorZona = {
    "Zona C": ["SR INSURGENTES SEARS","SR CONDESA RESTAURANTE","SR CONDESA DELIVERY", "IZK DEL VALLE","SR POLANCO","IZK REFORMA"],
    ...
    try {
        ...
        if(data.ok) {
            ...
            usuariosUnicos.forEach(u => {
                ...
                const retardos = regs.filter(r => r[4] > "09:10").length; // Conteo de retardos

                body.innerHTML += `
                    <tr>
                        <td class="fw-bold">${u}</td>
                        <td class="text-danger">--</td>
                        <td class="text-warning fw-bold">${retardos}</td> <td class="text-success fw-bold">${asistencias}</td>
...
function verDetalleQuincena(u) {
    ...
    registros.forEach(r => {
        ...
        lista.innerHTML += `<li class="list-group-item d-flex justify-content-between align-items-center border-dark mb-1"><div><div class="fw-bold">${r[0]}</div><div class="small">Entrada: ${r[4]}</div></div>${status}</li>`;
    });
            ...
            sucursales.forEach(s => {
                ...
                cont.innerHTML += `
                    <div class="card mb-3 border-dark"><div class="row g-0 align-items-center">
                        <div class="col-4 p-2"><img src="${foto}" class="img-fluid rounded border border-dark" style="height:110px; width:100%; object-fit:cover;"></div>
                        <div class="col-8"><div class="card-body py-2">
                            ...
                            <p class="small text-muted mb-1"><i class="bi bi-geo-alt"></i> Ubicación activa</p>
